...
function Header({ onButtonClick, showProfile }: any) {
  return (
    <header className="w-full px-4 sm:px-6 lg:px-20 py-4 sm:py-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-2">
          hype<span className="text-purple-500">Tracker</span>
        </h1>
        <p className="text-sm sm:text-base">Today's NBA games sorted by hype rating</p>
      </div>
      <button
        onClick={onButtonClick}
        className="px-4 sm:px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg transition-colors flex items-center gap-2 text-sm sm:text-base"
      >
        {showProfile ? (
          <>
...
function GameCard({ game, rank, favoriteTeam, timezone = 'America/New_York' }: any) {
  ...
  const getStreak = (teamName: string) => {
    return (window as any).teamStreaks?.[teamName] || null;
  };

  const formatStreak = (streak: string | null) => {
    if (!streak) return null;
    const isWin = streak.startsWith('W');
    return (
      <span className={`ml-1 text-xs ${isWin ? 'text-green-400' : 'text-red-400'}`}>
        {streak}
      </span>
    );
  };

  const hasFavoriteTeam = favoriteTeam === game.home.name || favoriteTeam === game.away.name;

  return (
    <div className={`bg-gray-900 border rounded-lg p-4 sm:p-6 hover:border-purple-500 transition-all ${hasFavoriteTeam ? 'border-purple-600 ring-2 ring-purple-500/50' : 'border-gray-800'}`}>
      {hasFavoriteTeam && (
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="text-2xl sm:text-3xl font-bold text-purple-500">#{rank}</div>
          <div className="text-xs sm:text-lg font-bold text-purple-500">{getHypeLabel(game.hype || 0)}</div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4 items-center">
        <div className="text-right">
          <div className={`text-sm sm:text-xl font-bold ${favoriteTeam === game.away.name ? 'text-purple-400' : ''} truncate`}>
            {game.away.name}
          </div>
          <div className="text-gray-400 text-xs sm:text-sm">
            {game.away.wins}-{game.away.losses}
            {formatStreak(getStreak(game.away.name))}
          </div>
        </div>
        ...
        <div className="text-left">
          <div className={`text-sm sm:text-xl font-bold ${favoriteTeam === game.home.name ? 'text-purple-400' : ''} truncate`}>
            {game.home.name}
          </div>
          <div className="text-gray-400 text-xs sm:text-sm">
            {game.home.wins}-{game.home.losses}
            {formatStreak(getStreak(game.home.name))}
          </div>
        </div>
      </div>
    </div>
...
export default function HypeTracker() {
  ...
  useEffect(() => {
    const loadStandings = async () => {
      try {
        ...
        const eastData = parse(data.children.find((c: any) => c.name === "Eastern Conference"));
        const westData = parse(data.children.find((c: any) => c.name === "Western Conference"));

        setEast(eastData);
        setWest(westData);

        const streakMap: any = {};
        [...eastData, ...westData].forEach((team: any) => {
          streakMap[team.name] = team.streak;
        });
        (window as any).teamStreaks = streakMap;
      } catch (err) {
  ...
  useEffect(() => {
    const loadGames = async () => {
      try {
        ...
        localStorage.setItem(cacheKey, JSON.stringify({ games: freshGames, date: today }));
  ...
  if (games.length === 0) {
    return (
      <div className="min-h-screen bg-black text-white">
        <Header onButtonClick={() => router.push(user ? '/profile' : '/login')} showProfile={!!user} />
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      <div className="fixed inset-0 bg-gradient-to-br from-purple-900/20 via-black to-purple-900/20" />
      ...
      <div className="relative z-10 h-screen overflow-y-auto scrollbar-hide">
        ...
        <div className="px-4 sm:px-6 lg:px-20 mb-4">
          <button
          ...
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
            <div className="bg-zinc-900 rounded-lg max-w-2xl w-full max-h-[80vh] overflow-auto">
              <div className="sticky top-0 bg-zinc-900 border-b border-zinc-800 p-4 flex justify-between items-center">
                ...
                <button onClick={() => setShowStandings(false)} className="text-gray-400 hover:text-white">
        ...
        <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-20 py-4 space-y-4">
